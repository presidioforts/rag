Perfect — here’s a **drop-in, executable** `rag_gate.py` with the agreed improvements baked in (no reranker, same stack). It’s production-hardened for MVP and keeps latency low.
...
def _freshness_weight(md: Dict[str, Any]) -> float:
    ...
    if not ts:
        return 1.0
    try:
        # Accept isoformat or epoch seconds
        if isinstance(ts, (int, float)):
            updated = datetime.fromtimestamp(float(ts), tz=timezone.utc)
        else:
...
def multi_signal_gate(query: str, candidates: List[Chunk]) -> GateResult:
    ...
    try:
        ...
        # Gate decision: require coverage + (consensus OR literals_ok); if literals exist, literals_ok must be True
        if (not consensus and not literals_ok) or (literals and not literals_ok):
...
**What’s in this version (A+ core):**

* Robust **z-score + margin** with freshness prior (optional).
* **Coverage** over top-N union with IDF.
* **Consensus** requires ≥2 chunks from the same section, each covering ≥⅓ salient terms (min 1).
* **Exact-match**: if literals exist (e.g., `OOMKilled`, quoted lines), require a literal hit; literals can satisfy consensus on their own.
* **No metadata mutation** (z-scores kept local).
* **MMR-lite** diversity via per-section cap; docstring shows how to flip to true MMR retriever later.
* **Security/robustness**: query/candidate caps, tokenization safety, defensive fallbacks.
* **Telemetry**: a single structured `logger.info("rag_gate_decision", ...)` at each return with z1, margin, coverage, consensus, literals flags, and reason on DEFLECT.

Drop this in as `rag_gate.py`, wire `answer_with_gate(vectordb, query)` into your handler, and you’re set to test.
